package labs.simplemovieserver.database
...
* */
class PsqlMovieDAO(initialMovie: MovieWithoutId, dbHost: String, dbUser: String, dbPassword: String, dbName: String)(implicit ec: ExecutionContext) extends MovieDao {
  ...
  Await.result(
    for {
      ...
      _ <- if(allMovies.exists(m => m.rating == initialMovie.rating && m.title == initialMovie.title && m.year == initialMovie.year)) Future.successful()

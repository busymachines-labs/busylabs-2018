...
object PureMovieServerApp extends StreamApp[IO] {
  ...
  override def stream(args: List[String], requestShutdown: IO[Unit]): Stream[IO, StreamApp.ExitCode] = {
    ...
    for {
      ...
      imdbConfig <- Stream.eval(IMDBAlgebraConfig.default[IO])
      ...
      pmsModule    <- Stream.eval(pureMovieServerModule[IO](imdbConfig, gmailConfig, transactor))
  ...
  private def pureMovieServerModule[F[_]: Concurrent](imdbConfig: IMDBAlgebraConfig,gmailConfig: GmailConfig, transactor: Transactor[F]): F[ModulePureMovieServer[F]] =
    Concurrent.apply[F].delay(ModulePureMovieServer.concurrent(imdbConfig, gmailConfig)(implicitly, transactor))
